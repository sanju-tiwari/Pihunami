...
import movie from '../../api/anime.json';
...
function Subscrip() {
  ...
  useEffect(() => {
    const product = movie.find((p) => p.id === parseInt(ids));
                  ...
                  <div className=' h-[5rem] md:h-[3rem] w-full mt-[1rem]  ' >
                    <div className='flex md:items-center justify-between md:flex-row flex-col items-start ' >
                      ...
                      <div className='flex items-center gap-[1rem] mt-1 md:mt-0 justify-center   h-[2rem] md:h-[3rem] w-[16.5rem] md:w-[20rem]  ' >
                          ...
                          <span>/</span>
                          <input  value={fill.expiryYear}  onChange={(e)=>setfill({...fill, expiryYear:e.target.value})}   className='h-[2rem] md:h-[3rem] rounded-[10px] border-[1px] ease-in-out duration-150 border-gray-400 w-[7rem] hover:bg-[#2F2933] hover:border-[#8f879d] placeholder:text-gray-300 text-center focus:outline-0 ' type="text" placeholder='327' />
                      </div>
                    </div>
